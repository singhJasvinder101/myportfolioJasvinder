...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const link = document.querySelector('#hero-btns a');
    ...
    navButton.addEventListener('click', () => {
        ...
        closeLists.style.display = (closeLists.style.display === 'none') ? 'block' : 'none';
    });

    // Close mobile navigation menu when a section is clicked
    sections.forEach((section) => {
        section.addEventListener('click', () => {
            links.forEach((link) => {
                link.classList.remove('active');
            });
            closeLists.style.display = 'none';
        });
    });
});